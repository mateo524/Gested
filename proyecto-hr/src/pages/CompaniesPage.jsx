...
export default function CompaniesPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const tempPassword = data.adminUser?.temporaryPassword
        ? ` Password temporal: ${data.adminUser.temporaryPassword}`
        : "";
      setMessage(`Empresa creada.${tempPassword}`);
      await loadCompanies();
    ...
    <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-slate-500">
          Crea una empresa y deja listo su acceso inicial desde el alta.
        </p>
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          <input
            className="w-full rounded-2xl border border-slate-300 px-4 py-3"
            type="password"
            placeholder="Password inicial del admin (opcional)"
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-slate-500">
          Cada empresa conserva sus usuarios, roles, auditoria y contenido aislados.
        </p>
