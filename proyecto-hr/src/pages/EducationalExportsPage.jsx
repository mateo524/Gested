  ...
  async function downloadDataset(format) {
    try {
      const response = await fetch(
        `${apiUrl}/education-exports/download/${dataset}${queryString ? `${queryString}&format=${format}` : `?format=${format}`}`,
        {
          headers: {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
        <p className="text-sm uppercase tracking-[0.22em] text-emerald-500">Información institucional</p>
        <h3 className="mt-3 text-3xl font-bold text-slate-950">Bases y Descargas</h3>
        <p className="mt-3 max-w-3xl text-slate-500">
          Consulta y descarga empleados, evaluaciones, métricas y planes de desarrollo con filtros
          y permisos específicos para cada rol.
        </p>
      </section>
      ...
      {overview ? (
        <section className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          ...
          <SummaryCard label="Métricas" value={overview.summary.metrics} hint="Indicadores activos" />
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-slate-500">Vista previa limitada según tu rol y tus permisos.</p>
            </div>
              ...
              <thead>
                <tr className="border-b border-slate-200 text-slate-500">
                  ...
                  {dataset === "metrics" ? (
                    <>
                      <th className="px-4 py-3 font-medium">Nombre</th>
                      <th className="px-4 py-3 font-medium">Ponderación</th>
                  ...
                  <tr key={item._id} className="border-b border-slate-100">
                    ...
                    {dataset === "metrics" ? (
                      <>
                        ...
                        <td className="px-4 py-3 text-slate-600">{item.activa ? "Sí" : "No"}</td>
                      </>
                    ) : null}
                    {dataset === "developmentPlans" ? (
                      <>
            ...
            {(overview?.recentDownloads || []).length ? (
              overview.recentDownloads.map((download) => (
                <article key={download._id} className="rounded-[1.75rem] border border-slate-200 p-4">
                  <p className="font-semibold text-slate-950">{datasetLabels[download.exportType] || download.exportType}</p>
              ...
              <p className="text-slate-500">Todavía no hay descargas registradas.</p>
            )}
          </div>
        </div>
      </section>
