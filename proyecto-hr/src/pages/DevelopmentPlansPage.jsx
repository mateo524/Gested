...
export default function DevelopmentPlansPage() {
  ...
  const cargarPlanes = async (intento = 0) => {
    try {
      setCargando(true);
      setError(null);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/development-plans`, {
      ...
      if (intento < 2) {
        setTimeout(() => cargarPlanes(intento + 1), 1500);
        return;
      }
  ...
  const handleDeletePlan = async (planId) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/development-plans/${planId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  if (selectedPlan) {
    return (
      <DevelopmentPlanDetail
        ...
        onUpdate={() => { cargarPlanes(); setSelectedPlan(null); }}
      />
    );
  }
  ...
  if (showForm) {
    return (
      <DevelopmentPlanForm
        plan={editingPlan}
        onVolver={() => { setShowForm(false); setEditingPlan(null); }}
        onSave={() => { cargarPlanes(); setShowForm(false); setEditingPlan(null); }}
      />
    );
  }
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
        ...
        {error && (
          <div className="text-center py-8">
            ...
            <button onClick={() => cargarPlanes()} className="text-emerald-600 hover:text-emerald-800 font-semibold text-sm">
              Reintentar
            </button>
          </div>
        )}
        ...
        {!cargando && !error && planes.length === 0 && (
          <p className="text-slate-500 text-center py-8">No hay planes de desarrollo registrados</p>
        )}
              ...
              <div key={plan._id} onClick={() => setSelectedPlan(plan)}
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${
                    plan.estado === "en_curso" ? "bg-blue-100 text-blue-800"
                    : plan.estado === "completado" ? "bg-emerald-100 text-emerald-800"
                ...
                <div className="mb-3">
                  ...
                  <div className="w-full bg-slate-200 rounded-full h-2">
                    <div className="bg-emerald-500 h-2 rounded-full transition-all" style={{ width: `${plan.progreso}%` }} />
                  </div>
                </div>
                <p className="text-xs text-slate-500">Empleado: {plan.empleadoEmail}</p>
                <div className="flex gap-2 mt-3">
                  <button onClick={(e) => { e.stopPropagation(); setEditingPlan(plan); setShowForm(true); }}
